...
exports.signup = async (req, res) => {
  try {
    const { firstName, lastName, email, password, accountType, token } = req.body;
    ...
    // Worker-specific check: validate token and find the Mechanic (owner)
    if (accountType === "Worker") {
      if (!token) {
        return res.status(400).json({
          success: false,
          message: "Token is required for Worker registration.",
        });
      }

      const owner = await User.findOne({ uniqueToken: token, accountType: "Mechanic" });

      if (!owner) {
        return res.status(400).json({
          success: false,
          message: "Invalid or expired token. Mechanic not found.",
        });
      }
    }
    ...
    res.status(200).json({
      ...
      tempUser: {
        ...
        password,
        accountType,
        token, // included for Worker account type
      },
    });
  } catch (error) {
    res.status(500).json({
